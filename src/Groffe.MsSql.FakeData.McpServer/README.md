...
{
  "servers": {
    "Groffe.MsSql.FakeData.McpServer": {
      "type": "stdio",
      "command": "dotnet",
      "args": [
        "run",
...
Once configured, you can ask Copilot Chat for a random number, for example, `Give me 3 random numbers`. It should prompt you to use the `get_random_number` tool on the `Groffe.MsSql.FakeData.McpServer` MCP server and show you the results.
